...
@Component({
  selector: 'app-reschedule-assigned-quiz',
  templateUrl: './reschedule-assigned-quiz.component.html',
  styleUrls: ['./reschedule-assigned-quiz.component.css']
})
export class RescheduleAssignedQuizComponent implements OnInit {
  ...
  editAssignedQuiz() {
    ...
    if ((Date.parse(postData['startDate'])) > (Date.parse(postData['endDate']))) {
      ...
      //removing "quizId"
      delete this.addNewQuizForm.value.quizId;
